...
| `lighthouse-ci.yml` | PR touching `internal/web/**`, `.lighthouserc.json`, `tests/lighthouse/**`, or the workflow itself | Lighthouse CI performance budget against `agent-deck web --no-tui`. Hard gates on `total-byte-weight`, `resource-summary:script:size`, and `cumulative-layout-shift`; soft warnings on FCP/LCP/TBT/Speed Index. Reinstated in v1.7.70 after the `--no-tui` flag fixed the bubbletea/headless-CI start failure that disabled the gate in v1.7.42. Recalibrate with `./tests/lighthouse/calibrate.sh` when bundle size changes. |
